...
const ReportExporter = ({
  ...
  const handleExportSelected = async () => {
    ...
    setIsExporting(true);
    try {
      const response = await reportService.exportReportsToPDF(
        selectedReports,
        exportOptions
      );
      ...
      toast.success(
        `Successfully exported ${selectedReports.length} report(s) to PDF`
      );
      onExportComplete();
      ...
      if (error.response) {
        if (error.response.status === 404) {
          errorMessage =
            "Export feature not available. Please check if the backend is running.";
  ...
  const handleExportAll = async () => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `all_reports_export_${
        new Date().toISOString().split("T")[0]
      }.pdf`;
      document.body.appendChild(link);
      link.click();
      ...
      toast.success(
        `Successfully exported ${allReports.length} report(s) to PDF`
      );
      onExportComplete();
      setShowExportOptions(false);
      onExportModeChange(false); // Reset export mode after export
  ...
  const handleSingleReportExport = async (reportId) => {
    setIsExporting(true);
    try {
      const response = await reportService.exportSingleReportToPDF(
        reportId,
        exportOptions
      );
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `report_${reportId}_${
        new Date().toISOString().split("T")[0]
      }.pdf`;
      document.body.appendChild(link);
      link.click();
                  ...
                  <label className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-300 transition-colors cursor-pointer">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        Complete information with all selected content
                      </p>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-300 transition-colors cursor-pointer">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        Basic information and key details
                      </p>
                    </div>
                  </label>
                </div>
